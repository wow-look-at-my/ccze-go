...
// WriteColored writes text to w wrapped in ANSI escape sequences matching
...
func (t *Table) WriteColored(w io.Writer, col Color, text string) {
	...
	if bg > 0 || !t.transparent {
		fmt.Fprintf(w, "\x1b[%dm", ansiColor[bg]+10)
	}

	// Foreground color + text + reset
	fg := cv & 0xf
	fmt.Fprintf(w, "\x1b[%dm%s\x1b[0m", ansiColor[fg], text)
}

// WriteSpace writes a space character colored with the Default color.
func (t *Table) WriteSpace(w io.Writer) {
	t.WriteColored(w, Default, " ")
}
