...
func (p *DpkgPlugin) Handle(line string) (bool, string) {
	// Try status line (DFA pre-filter for fast rejection)
	if matchers.MatchDpkgStatus(line) {
	...
	// Try action line (DFA pre-filter for fast rejection)
	if matchers.MatchDpkgAction(line) {
	...
	// Try conffile line (DFA pre-filter for fast rejection)
	if matchers.MatchDpkgConffile(line) {
